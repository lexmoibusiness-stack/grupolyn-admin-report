...
function deployOne_(url, scriptId, stubSource) {
  try {
    ...
    if (scriptId) {
      ...
      // Projects this deployer created also carry its onOpen file: keep it current too.
      var ownsTrigger = (content.files || []).some(function (f) { return f.name === 'LynAdminTrigger'; });
      if (ownsTrigger) files = upsertFile_(files, { name: 'LynAdminTrigger', type: 'SERVER_JS', source: triggerSource_() });
    ...
    api_('put', '/' + created.scriptId + '/content', {
      files: [
        ...
        { name: 'LynAdminTrigger', type: 'SERVER_JS', source: triggerSource_() }
      ]
    });
...
function patchManifest_(source) {
  ...
  manifest.exceptionLogging = 'STACKDRIVER'; // otherwise trigger failures leave no log at all
...
/** onOpen of the injected project: builds the Admin menu and always logs why if it cannot. */
function triggerSource_() {
  return [
    '/** GENERATED by the LyN deployer. Builds the Admin menu when the spreadsheet opens. */',
    'function onOpen(e) {',
    '  try {',
    '    ' + DEPLOY.librarySymbol + '.lynAdminBuildMenu();',
    '  } catch (err) {',
    "    console.error('LynAdmin onOpen failed: ' + err + '\\n' + (err && err.stack));",
    '  }',
    '}',
    ''
  ].join('\n');
